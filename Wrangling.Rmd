...
```{r}
head(heart_disease, 6)
tail(heart_disease, 3)
```
...
```{r}
heart_disease_ad <- select(heart_disease, Age, Drugs)
```
...
To select a range of columns by name (that are in consecutive order), use the `:` (colon) operator. For example, view the output from uncommentting the following line of code:
...
We can also select the rows/observations that satisfy certain criteria. Try selecting the rows with more than 500 duartion:

```{r}
duration_500 <- filter(heart_disease, Duration %in% c(500))
```

We can also filter on mutiple criteria. Select rows with `Age` above 60 and the `gender` is 'Male':

```{r}
men_60 <- filter(heart_disease, Age %in% c(60), Gender %in% c('Male'))
```
...
```{r}
heart_disease %>%
  select(Age, Cost, ERVisit, Duration) %>%
  filter(Age %in% 60) %>%
  arrange(desc(Duration))
```
...
heart_disease <- heart_disease %>%
  mutate(cost_per_day = Cost / Duration)
```
...
```{r}
heart_disease %>%
  summarize(mean(Cost), min(Cost), max(Cost))
```
